...
--Douglas Kihara
-- Que mostre todos os clientes que locaram filmes com valor maior ou igual ao valor da categoria com maior valor.

SELECT CLIENTE.NOME, LOCACAO_FILME.VALOR AS 'VALOR LOCAÇÃO', CATEGORIA.VALOR AS 'VALOR CATEGORIA' FROM CLIENTE
INNER JOIN LOCACAO ON CLIENTE.COD_CLI = LOCACAO.FK_CLI
INNER JOIN LOCACAO_FILME ON LOCACAO_FILME.FK_LOC = LOCACAO.COD_LOC
INNER JOIN FILMES ON LOCACAO_FILME.FK_FILME = FILMES.COD_FILME
INNER JOIN CATEGORIA ON FILMES.FK_CAT = CATEGORIA.COD_CAT
WHERE LOCACAO_FILME.VALOR >= CATEGORIA.VALOR
ORDER BY LOCACAO_FILME.VALOR DESC;

----------------------------------------------------------------------------------------------------------------------

-- Que mostre a lista de gêneros, juntamente com o último filme locado de cada gênero.

SELECT TOP 1 WITH TIES FILMES.TITULO, GENERO.NOME AS 'GERENO', LOCACAO.DATA_LOC FROM GENERO
INNER JOIN FILMES ON FILMES.FK_GEN = GENERO.COD_GEN
INNER JOIN LOCACAO_FILME ON LOCACAO_FILME.FK_FILME = FILMES.COD_FILME
INNER JOIN LOCACAO ON LOCACAO_FILME.FK_LOC = LOCACAO.COD_LOC
GROUP BY FILMES.TITULO, GENERO.NOME, LOCACAO.DATA_LOC
ORDER BY ROW_NUMBER() OVER (PARTITION BY GENERO.NOME ORDER BY MAX(LOCACAO.DATA_LOC) DESC);

----------------------------------------------------------------------------------------------------------------------

-- Que traga os filmes locados em um determinado período.

SELECT FILMES.TITULO, YEAR(LOCACAO.DATA_LOC) AS 'Ano' FROM FILMES
INNER JOIN LOCACAO_FILME ON LOCACAO_FILME.FK_FILME = FILMES.COD_FILME
INNER JOIN LOCACAO ON LOCACAO.COD_LOC = LOCACAO_FILME.FK_LOC
WHERE YEAR(LOCACAO.DATA_LOC) BETWEEN 2015 AND 2018;

----------------------------------------------------------------------------------------------------------------------

-- Que mostre todos os filmes já locados e a quantidade de vezes que cada um foi locado.

SELECT FILMES.TITULO, COUNT(LOCACAO_FILME.FK_FILME) AS 'N Alocado' FROM FILMES
INNER JOIN LOCACAO_FILME ON FILMES.COD_FILME = LOCACAO_FILME.FK_FILME
INNER JOIN LOCACAO ON LOCACAO.COD_LOC = LOCACAO_FILME.FK_LOC
GROUP BY FILMES.TITULO
ORDER BY 'N Alocado' DESC;

----------------------------------------------------------------------------------------------------------------------

-- Que mostre os 2 filmes mais locados no ano de 2022.

SELECT FILMES.TITULO, COUNT(LOCACAO_FILME.FK_FILME) AS 'N Alocado', YEAR(LOCACAO.DATA_LOC) AS 'Ano' FROM FILMES
INNER JOIN LOCACAO_FILME ON FILMES.COD_FILME = LOCACAO_FILME.FK_FILME
INNER JOIN LOCACAO ON LOCACAO.COD_LOC = LOCACAO_FILME.FK_LOC
WHERE YEAR(LOCACAO.DATA_LOC) = 2022
GROUP BY FILMES.TITULO, YEAR(LOCACAO.DATA_LOC)
ORDER BY 'N Alocado' DESC;
...
SELECT TOP 1 GENERO.NOME, COUNT(LOCACAO_FILME.FK_FILME) AS 'N Alocado' FROM GENERO
INNER JOIN FILMES ON FILMES.FK_GEN = GENERO.COD_GEN
INNER JOIN LOCACAO_FILME ON FILMES.COD_FILME = LOCACAO_FILME.FK_FILME
INNER JOIN LOCACAO ON LOCACAO.COD_LOC = LOCACAO_FILME.FK_LOC
GROUP BY GENERO.NOME
ORDER BY COUNT(LOCACAO.COD_LOC) DESC;

----------------------------------------------------------------------------------------------------------------------

-- Que conte os clientes por estados.

SELECT ENDERECO.UF, COUNT(CLI_ENDERECO.FK_CLI) AS 'NUM CLIENTES' FROM CLIENTE
INNER JOIN CLI_ENDERECO ON CLIENTE.COD_CLI = CLI_ENDERECO.FK_CLI
INNER JOIN ENDERECO ON ENDERECO.COD_END = CLI_ENDERECO.FK_END
GROUP BY ENDERECO.UF
